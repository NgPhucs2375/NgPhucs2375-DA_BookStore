-- Align enum-backed columns to VARCHAR to match Hibernate's VARCHAR handling on SQL Server

IF OBJECT_ID('books', 'U') IS NOT NULL
BEGIN
  -- Step 1: Drop the default constraint if it exists
  DECLARE @default_constraint_name_books VARCHAR(255);
  ...
  DECLARE @check_constraint_name VARCHAR(255);
  ...
  ALTER COLUMN approval_status VARCHAR(20) NOT NULL;
END;
